 ...
 *
 */
public class TrendBuyV4Model implements StockModel {
	...
	private Map middleLayerStock = new HashMap();
	private Map buyDate = new HashMap();
	...
	public boolean hasSell(float costPrice) {
		...
		List<String> range = DateUtil.getRange(stockId, (String) buyDate.get(stockId), date);
		if(range.size()>=Params.holdPeriod){
			buyDate.remove(stockId);
			return true;
		} else {
			return false;
		}
