...
public class ThirdBuyModel implements StockModel {
	...
	public void stockAnalysis() {
		List result = StockPriceWareHouse.getStockInfoList(stockId, DateUtil
				.getRange(stockId,
						DateUtil.getBackdate(date, Params.TB_N_DAYS, stockId),
						date));
		...
		if (lastPeak.isHigh) {
			...
			Peak virtualPeak = new Peak(temp[0], Double.parseDouble(temp[3]),
					false);
		...
		for (int i = 0; i < peakList.size(); i++) {
			...
			logger.debug(temp.getDate() + "," + temp.getPrice() + ","
					+ temp.getIsHigh());
		}
		logger.debug("--------------------------完美分割线---------------------------");
		...
		for (int i = 0; i < peakList.size() - 1; i++) {
			...
			for (int j = 0; j < row.size(); j++) {
				...
				if ((temp[0].compareTo(trend.getBeginDate()) >= 0)
						&& (temp[0].compareTo(trend.getEndDate()) <= 0)) {
					if ((trend.getIsRise())
							&& (Double.parseDouble(temp[5]) > 0)) {
		...
		if (trendList.size() > 0 && bigZsList.size() > 0) {
			if ((trendList.get(trendList.size() - 1).getHighPrice() == bigZsList
					.get(bigZsList.size() - 1).getHighPrice())
					|| (trendList.get(trendList.size() - 1).getLowPrice() == bigZsList
							.get(bigZsList.size() - 1).getLowPrice())) {
	...
	public boolean hasSell(float costPrice) {
		...
		if (((!(Params.TB_SELL_ZS_OR_MAX == 1) || ((lastZs.getHighPrice()
				* Params.TB_SELL_PERCENTS >= lastClose))) // 下跌时候跌破前高点的百分比
		&& (!(Params.TB_SELL_ZS_OR_MAX == 2) || ((lastZs.getZsHighPrice()
				* Params.TB_SELL_PERCENTS >= lastClose)))// 下跌时候跌破前中枢的中枢高点的百分比
		)
				|| (!(Params.TB_SELL_BEILI) || (((!lastPeak.isHigh)
						&& (lastHighTrend.getMacdVol() <= lastZs.getZsTrend()
								.getMacdVol()) && (lastHighTrend.getHighPrice() > lastZs
						.getHighPrice())))) // （前一天已经出现上涨的高点）出现背离：！lastPeak.isHigh说明已经有回落了
		) {
			return true;
		}
		return false;
	...
	public boolean hasBuy() {
		...
		if (last2Zs.getZsTrend() != null) {
			if (((!true) || (lastZs.getZsTrend().getIsRise())) // 上一个zs前的趋势必须是上涨的（必须条件）
					&& ((!true) || (lastPeak.getPrice()
							* Params.TB_BACK_PERCENTS > lastClose)) // 当未实现下一级中枢判断之前（必须条件），计算最高点下来的比例
					&& ((!true) || (lastSecTrend.getMacdVol() > lastZs
							.getZsTrend().getMacdVol())) // 这次上涨突破趋势比上一个中枢前的上涨突破趋势相比未背离
					...
					&& (!(Params.TB_FIRST_TB) || (!last2Zs.getZsTrend()
							.getIsRise())) // 取第一个三买，即前一个中枢是上涨，再上一个中枢是下跌的
					&& (!(Params.TB_TREND_RISE_PERCENTS > 1) || (lastSecTrend
							.getHighPrice() < lastSecTrend.getLowPrice()
							* Params.TB_TREND_RISE_PERCENTS)) // 这次上涨突破趋势的涨幅不能太大。
			) {
				return true;
			}
		}
